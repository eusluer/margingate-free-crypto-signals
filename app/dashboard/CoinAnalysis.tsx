...
export default function CoinAnalysis() {
  const { data: signalsData } = useSWR<SignalsJson>(getSignalsUrl('en'), fetchJson, { 
    refreshInterval: 300_000,
    revalidateOnFocus: true,
    revalidateOnReconnect: true,
    dedupingInterval: 60_000
  });
  const { data: coinsData } = useSWR<CoinsJson>(getCoinsUrl('en'), fetchJson, { 
    refreshInterval: 300_000,
    revalidateOnFocus: true,
    revalidateOnReconnect: true,
    dedupingInterval: 60_000
  });
  const { data: alarmsData } = useSWR<AlarmsJson>(getAlarmsUrl('en'), fetchJson, { 
    refreshInterval: 300_000,
    revalidateOnFocus: true,
    revalidateOnReconnect: true,
    dedupingInterval: 60_000
